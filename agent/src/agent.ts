...
const toolDefinitions = [
  {
    ...
    description:
      "Retrieves the candidate's name and resume summary. Call this first before speaking.",
    parameters: {
  ...
  },
  {
    ...
    description:
      "Fetches the next question in the interview path. Returns null when all questions are done.",
    parameters: {
      type: "object" as const,
      properties: {
  ...
  },
  {
    ...
    description:
      "Called when all questions are answered or the candidate ends the session. Saves transcript and triggers evaluation.",
    parameters: {
      type: "object" as const,
      properties: {
        candidateId: { type: "string" as const },
        interviewId: { type: "string" as const },
        fullTranscript: { type: "string" as const },
        hintsUsed: {
          type: "array" as const,
...
async function handleToolCall(
  ...
  switch (name) {
    case "fetch_candidate_context":
      return fetchCandidateContext(
        (args.candidateId as string) || config.candidateId
      );

    case "fetch_next_question":
      return fetchNextQuestion(
        (args.jobId as string) || config.jobId,
        args.currentStep as number
      );

    case "conclude_interview":
      return concludeInterview({
        interviewId: (args.interviewId as string) || config.interviewId,
        fullTranscript: args.fullTranscript as string,
        hintsUsed: (args.hintsUsed as object[]) || [],
      });
...
export async function createInterviewAgent(
  ...
  console.log(
    `[Agent] System prompt loaded (${SYSTEM_PROMPT.length} chars)`
  );
  ...
  console.log(
    "[Agent] Agent scaffold ready. Install @livekit/agents and configure the multimodal pipeline to enable live interviews."
  );
}
